...
$(function(){
	...
	$(Allchk).on('change', function(){
		if (Allchk.prop('checked')) {
			$('.termsLabel').addClass('on').find('input').prop('checked',true);
			$('.termsLabel').not('.all').find('label').html('동의 완료');
			$('.termsList > li').addClass('on fix');
	...
	$('.termsList').find('.termsLabel').not('.all').children('input').each(function(){
		$(this).on('click', function(){
			...
			if ( $(this).prop('checked') )
			{
				...
				termsLabelLng = $('.termsLabel').not('.termsLabel.exception, .termsLabel.all').length;
				termsLabeldoneLng = $('.termsLabel.on').not('.termsLabel.exception, .termsLabel.all').length;
				termsLabelLngAll = $('.termsLabel').not('.termsLabel.all').length;
				termsLabeldoneLngAll = $('.termsLabel.on').not('.termsLabel.all').length;
...
function numHangul(event) {
	...
	var result = '';
	var manCount = 0; // 만단위 0이 아닌 금액 카운트
